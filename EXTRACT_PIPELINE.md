...
- Box dimensions and region layout are derived from config. The only values stored for extract layout are **two scale factors**: one for regular screenshots and one for blueprint (`extract.regular_scale`, `extract.blueprint_scale` in `config.yaml`). The front-end config UI lets users tune these so region overlays align; users copy the values into `config.yaml` as needed. Augmentation shift bounds (positive/negative x and y for regular and blueprint) are also in `config.yaml` under `extract.augment`.
