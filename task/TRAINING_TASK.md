...
- **Long-running slot (one thread):** One worker thread consumes from **both** `training_tasks` and `recommendation_tasks` (`brpop([training_tasks, recommendation_tasks], ...)`). At most one training or one recommendation runs at a time. The **same thread** also processes evaluation tasks from `evaluation_tasks`: when a training task is running and the evaluation queue is non-empty, training yields at the **next preview-epoch boundary** (checkpoint saved, GPU freed), the thread drains the evaluation queue, then training resumes from the checkpoint. This avoids GPU OOM by ensuring only one model is on GPU at a time. A key `training_current_task_id` identifies the active training task; `recommendation_current_task_id` the active recommendation task. When a new training (or recommendation) is created, only that type's current task is cancelled; the other type's queue is not cleared so the other type can wait in queue.
...
- The **task worker** runs a single long-running thread that processes training, recommendation, and evaluation (training yields at preview-epoch boundaries when evaluation tasks are pending); it updates Redis with status and results. The front-end polls `GET /api/tasks/{task_id}` to show progress and accuracy.
