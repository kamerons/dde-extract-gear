...
- **Loading:** All model loading and inference run in the task container. Each unit of work (one training run, one evaluation task, one recommendation task) runs in a **separate subprocess** (`task/run_task.py`); when the subprocess exits, GPU and process memory are fully released. The evaluation processor and training use **`_load_box_detector_model(load_path)`** (temp copy, format detection). Training suspends at preview-epoch boundaries when evaluation tasks are pending; the orchestrator drains evaluation (each in its own subprocess), then starts a new training subprocess to resume.
...
2. **Evaluate / preview** (task container only)  
   The client can call `POST /api/extract/training/evaluate` or `POST .../preview` to run a one-off evaluation task; the API enqueues it and returns `task_id`; the client polls `GET /api/tasks/{task_id}` for results and `model_format`. Each evaluation runs in its own **subprocess**; when evaluation tasks are pending during training, the training subprocess suspends at the next preview-epoch boundary (checkpoint saved, subprocess exits), the orchestrator runs each evaluation task in a subprocess, then starts a new training subprocess to resume. During training the subprocess builds preview at preview epochs and writes it to Redis. The frontend polls `GET /api/extract/training/preview/latest` to get the latest preview and display it; no manual "Run preview" is required.
