...
class ExtractGear:
  ...
  def run(self):
    ...
    for armor_type in ExtractGear.ARMOR_TYPES:
      ...
      armor_iterator.iterate(self.get_add_data_to_index_fn(armor_type, index, failed))
  ...
  def get_add_data_to_index_fn(self, armor_type, index, failed):
    def true_callback(gear_coord, page_num):
      return self.add_data_to_index(gear_coord, page_num, armor_type, index, failed)
    return true_callback


  def add_data_to_index(self, gear_coord, page, armor_type, index, failed):
      return self.recursive_add_to_index(0, gear_coord, page, armor_type, index, failed)


  def recursive_add_to_index(self, depth, gear_coord, page, armor_type, index, failed):
    ...
    data = self.card_reader.get_img_data(img, gear_coord)
    if not 'base' in data and depth >= ExtractGear.MAX_RECURSE:
      ...
      return self.recursive_add_to_index(depth + 1, gear_coord, page, armor_type, index, failed)
